...
> 에이전트 및 코파일럿을 위한 Unity_RecycleScroller 프로젝트 작업 가이드입니다.
...
### 검증 항목 추가 (사용자 선택)
- [ ] 이 문제를 `.guides/VERIFICATION_ITEMS.md`에 검증 항목으로 추가
  - 카테고리: (C# / Unity 컴파일 / 파일 이동 / UPM 패키지 / 파일 생성 / 기타)
  - 검증 항목: (다음 작업에서 확인해야 할 내용)
...
#### 1. 템플릿 파일 읽기
```
.wips/templates/WIP-Plan-YYYYMMDD-NNN.md
```
