// Unity ScrollRect 소스 코드 (참조용)
// 출처: UnityEngine.UI (com.unity.ugui)
//
// [참고 포인트 - 예정 작업 관련]
//
// 1. Elastic 핸들 사이즈 조정 (Task 1 참고)
//    - UpdateScrollbars() 메서드 (line ~510):
//      scrollbar.size = Clamp01((ViewSize - Abs(offset)) / ContentSize)
//      → offset(elastic 오버슈트량)만큼 핸들 사이즈가 줄어드는 원리
//
// 2. Elastic offset 계산 (Task 1 참고)
//    - CalculateOffset() / InternalCalculateOffset() 메서드:
//      content가 view 경계를 넘어간 양(offset)을 계산
//    - LateUpdate()에서 offset을 UpdateScrollbars()에 전달
//
// 3. RubberDelta (Task 1 참고)
//    - OnDrag()에서 Elastic 모드일 때 RubberDelta로 드래그 저항감 적용
//    - RubberDelta(overStretching, viewSize): 오버스트레칭 비율에 따른 감쇠 계산
//
// 4. MovementType 분기 패턴 (Task 2, 3 참고 - 객체화 대상)
//    - Unrestricted / Elastic / Clamped 세 가지 모드
//    - 현재 if/switch 분기 처리 → 전략 패턴 객체화 참고
...
namespace UnityEngine.UI
{
    ...
    public class ScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        // ★ 핵심: Elastic offset에 따라 scrollbar.size를 줄이는 로직
        ...
        // ★ 경계 초과량(offset) 계산 - Elastic/Clamped 공통
