...
> 관련 작업(파일 이동, rename, asmdef 변경 등) 시 참고하세요.
...
- [ ] 새로운 `using` 네임스페이스를 추가하거나 기본 클래스/인터페이스를 변경했을 때, 해당 타입이 속한 어셈블리가 `.asmdef`의 `references`에 명시적으로 포함되어 있는지 확인 (Unity가 컴파일 시 암묵적으로 참조를 포함시켜 빌드가 성공할 수 있지만, asmdef에 명시적 참조가 없으면 런타임에 타입 매핑이 실패하여 컴포넌트가 깨진다. 컴파일 성공만으로는 검증 불충분)
