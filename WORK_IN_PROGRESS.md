# 작업 추적 및 재개 시스템

> 작업 중단 시 이 파일을 사용하여 상태를 저장하고, 새 대화에서 재개합니다.

---

## 🔄 작업 재개 방법

### 새 대화에서 재개 시
```
1. 사용자: "재개: WIP-YYYYMMDD-NNN"
   또는
   사용자: "CONTINUE: WIP-YYYYMMDD-NNN"

2. 에이전트:
   - WORK_IN_PROGRESS.md 읽기
   - 해당 WorkID 상태 확인
   - 마지막 중단 시점 파악
   - "다음 작업" 확인
   - 즉시 재개

3. 재개 완료 후:
   - "🔄 WIP-YYYYMMDD-NNN 작업이 재개되었습니다"
   - "현재 단계: [완료된 단계]"
   - "다음 작업: [다음 작업]"
```

---

## 📊 현재 작업 목록

### 활성 작업 (진행 중)
| WorkID | 상태 | 유형 | 제목 | 시작일 | 진척도 |
|--------|------|------|------|--------|--------|
| WIP-20260322-001 | Plan | 수정 | UpdateCellView 매 프레임 분기 제거 — m_reverse/ScrollAxis 캐싱 | 2026-03-22 | 15% |

### 완료 작업 (히스토리)
| WorkID | 완료일 | 유형 | 제목 | 소요시간 |
|--------|--------|------|------|----------|
| WIP-20260312-001 | 2026-03-13 | 신규 | ScrollRect 코드 포크 기반 RecycleScroller 아키텍처 재구성 | - |
| WIP-20260214-001 | 2026-02-14 | 신규 | Elastic 핸들 사이즈 조정 동작 구현 | - |
| WIP-20260213-002 | 2026-02-14 | 수정 | 루프 스크롤바 핸들 위치 직접 배치 방식으로 재설계 | - |
| WIP-20260213-001 | 2026-02-13 | 수정 | RecycleScrollbar 핸들 최소 사이즈 보장 기능 추가 | - |

### 취소된 작업 (취소 히스토리)
| WorkID | 취소일 | 유형 | 제목 | 사유 |
|--------|--------|------|------|------|
| *(없음)* | - | - | - | - |

---

## 📝 작업 상세

> 이 섹션은 **작업 상세 문서의 구조 템플릿**입니다.
> 실제 작업 내용은 별도 문서(`.wips/active/` 하위)에 작성하세요.
> 작업 완료 후 해당 문서를 삭제하거나 `.wips/archive/`로 이동합니다.

### WIP-YYYYMMDD-NNN: [작업 제목]

#### 📋 계획 요약
- **유형**: [신규 B / 수정 A]
- **파일**: [대상 파일 목록]
- **변경 내용**: [변경 사항 요약]
- **시작일**: YYYY-MM-DD
- **문서 참조 순서**: CLAUDE.md(자동) → PROJECT_SUMMARY.md → WORK_IN_PROGRESS.md → 필요 시 AGENTS.md

#### 💾 상태 스냅샷
- **마지막 업데이트**: YYYY-MM-DD [단계명]
- **진척도**: XX%

#### ✅ 완료 단계
- [ ] 1. Plan (계획): 기획서 분석, 유형 판단, 계획 수립
- [ ] 2. Design (설계): 아키텍처 설계, 기술적 검증
- [ ] 3. Code (코딩): 코드 구현, 빌드 확인
- [ ] 4. Test (테스트): 컴파일 검증, 테스트 체크리스트 작성
- [ ] 5. Docs (문서화): CHANGELOG, 코드 주석
- [ ] 6. QA (품질검사): 코드 품질, SOLID 준수
- [ ] 7. Review (최종검토): architect 승인, 릴리즈 준비

#### 🚪 Validation Gates (검증 게이트)
- [ ] **Gate-1**: Plan → Design
- [ ] **Gate-2**: Design → Code
- [ ] **Gate-3**: Code → Test
- [ ] **Gate-4**: Test → Docs
- [ ] **Gate-5**: Docs → QA
- [ ] **Gate-6**: QA → Review
- [ ] **Gate-7**: Review → 완료

#### 🚧 진행 상황
```
[YYYY-MM-DD]: [타임스탬프 기록]
```

#### 🔗 관련 파일
- [관련 파일 목록]

#### 💬 사용자 메모
```
[사용자 메모]
```

---

## 🎯 WorkID 형식

### 추천 형식: **WIP-YYYYMMDD-NNN**

**구조:**
- `WIP`: Work In Progress (고정 접두사)
- `YYYYMMDD`: 날짜 (예: 20250202)
- `NNN`: 같은 날짜 내 순서, 3자리 0패딩 (001, 002, 003...)

**예시:**
```
WIP-20250202-001  # 2025년 2월 2일 첫 번째 작업
WIP-20250202-002  # 같은 날 두 번째 작업
WIP-20250203-001  # 다음 날 첫 번째 작업
```

**장점:**
- ✅ 날짜 기준으로 자동 정렬
- ✅ 충돌 가능성 최소화
- ✅ 히스토리 추적 용이
- ✅ 읽기 쉬움

---

## 🚨 긴급 명령어

### 작업 재개
```
재개: WIP-YYYYMMDD-NNN
또는
CONTINUE: WIP-YYYYMMDD-NNN
```

### 작업 완료
```
완료: WIP-YYYYMMDD-NNN
```
→ 완료 단계 모두 체크
→ 완료 작업으로 이동

### 작업 취소
```
취소: WIP-YYYYMMDD-NNN [사유]
```
→ 활성 작업에서 제거
→ 취소 작업으로 이동
→ 사유 기록

### 상태 확인
```
상태: WIP-YYYYMMDD-NNN
또는
상태: 전체
```

---

## 📝 작업 완료 시 처리

### 1. 완료 단계 확인
```markdown
- [x] 기획서 분석
- [x] 계획 수립
- [x] 사용자 확인
- [x] 구현 단계1
- [x] 구현 단계2
- [x] 빌드 테스트
- [x] 기능 테스트
- [x] 커밋
```

### 2. 정보 수집
```markdown
완료일: YYYY-MM-DD HH:MM
소요 시간: X시간
커밋 메시지: [태그] 요약
커밋 해시: abc123def456
```

### 3. 완료 작업으로 이동
```markdown
### 완료 작업 (히스토리)
| WorkID | 완료일 | 유형 | 제목 | 소요시간 |
|--------|--------|------|------|----------|
| WIP-YYYYMMDD-NNN | YYYY-MM-DD | 수정 | 작업 제목 | Xh |
```

### 4. 활성 작업에서 제거
```markdown
### 활성 작업 (진행 중)
| WorkID | 상태 | 유형 | 제목 | 시작일 | 진척도 |
|--------|------|------|------|--------|--------|
| *(삭제됨)* | - | - | - | - | - |
```

### 5. 상세 정보 보존 (히스토리용)
```markdown
### 완료된 작업 상세

#### WIP-YYYYMMDD-NNN: 작업 제목

**완료일:** YYYY-MM-DD HH:MM
**소요 시간:** X시간
**커밋:** abc123def456

[전체 내용 보존...]
```

---

## 🗑️ 작업 취소 시 처리

### 1. 취소 사유 기록
```
사용자: "취소: WIP-YYYYMMDD-NNN 우선순위 조정으로 인해"
```

### 2. 활성 작업에서 제거
```markdown
### 활성 작업 (진행 중)
| WorkID | 상태 | 유형 | 제목 | 시작일 | 진척도 |
|--------|------|------|------|--------|--------|
| *(삭제됨)* | - | - | - | - | - |
```

### 3. 취소 작업으로 이동
```markdown
### 취소된 작업 (취소 히스토리)
| WorkID | 취소일 | 유형 | 제목 | 사유 |
|--------|--------|------|------|------|
| WIP-YYYYMMDD-NNN | YYYY-MM-DD HH:MM | 수정 | 작업 제목 | 우선순위 조정 |
```

---

## 🔍 상태 확인 명령어

### 특정 작업 상태
```
상태: WIP-YYYYMMDD-NNN
```
→ 해당 WorkID의 상세 상태 출력

### 전체 상태
```
상태: 전체
```
→ 현재 활성 작업, 완료 작업, 취소 작업 모두 요약

---

## 📖 사용 예시

### 시나리오 1: 새 대화에서 작업 재개

```
사용자: "재개: WIP-YYYYMMDD-NNN"

에이전트:
🔄 WIP-YYYYMMDD-NNN 작업이 재개되었습니다.

현재 단계: 구현 단계2 (완료)
다음 작업: 빌드 테스트

관련 파일:
- Service.cs
- ViewModel.cs

진행하시겠습니까? (y/n)
```

---

## 🎯 에이전트 동작 가이드

### 작업 시작 시
1. 새로운 WorkID 생성: `WIP-YYYYMMDD-NNN` (최대 3번 재시도로 충돌 방지)
2. 활성 작업에 추가
3. 상세 정보 기록

### 진행 중 업데이트
1. 진행 상황에 타임스탬프 기록
2. 완료 단계 체크
3. 필요 시 사용자에게 진행 상황 보고

### Gate 진입/통과/실패 시 업데이트
1. Gate 진입 전 백업 저장
2. Gate 통과 시 스냅샷 저장
3. Gate 실패 시 에러 기록 및 롤백 준비

### 상태 복구 (롤백 시)
1. 백업 상태 확인
2. 스냅샷 상태로 복구
3. WORK_IN_PROGRESS.md 상태 업데이트

### 작업 완료 시
1. 완료 단계 모두 체크
2. 완료 정보 수집
3. 완료 작업으로 이동
4. 상세 정보 보존

### 작업 취소 시
1. 취소 사유 기록
2. 취소 작업으로 이동
3. 상세 정보 보존

---

## 📚 관련 문서

- [기획서 워크플로우](WORKFLOW_PLANNING/INDEX.md)
- [빠른 참조](QUICK_REFERENCE.md)
- [프로젝트 요약](PROJECT_SUMMARY.md)
