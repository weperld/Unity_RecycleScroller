...
1. **컴파일 검증**: Unity/C# 컴파일 에러 0건 확인 (LSP 또는 수동 코드 리뷰)
...
3. **전체 파이프라인** (Plan → Design → Code → Test → Docs → QA → Review) 및 Gate 검증
...
주요 명령어:
- `/project:신규 [기능 설명]` - 새로운 기능 추가 (파이프라인 적용)
- `/project:수정 [문제 설명]` - 버그 수정 또는 기능 개선 (파이프라인 적용)
- `/project:간편 [작업 설명]` - 파이프라인 생략, 최소 검증만 수행
- `/project:커밋` - 변경 사항 커밋 (메시지 자동 생성)
- `/project:전송` - 스테이징 → 커밋 → 푸시 한번에
- `/project:릴리즈 [버전]` - UPM 패키지 버전업 및 릴리즈
